...
export default function RecipePage() {
    ...
    const cuisineTypeDisplay = cuisineType.map((cuisine) => {
        return (
            <div key={cuisine} className={styles.cuisineTag}>
                {cuisine}
            </div>
        );
    });
    ...
    const dishTypeDisplay = dishType.map((dish) => {
        return (
            <div key={dish} className={styles.dishTag}>
                {dish}
            </div>
        );
    });
    ...
    const renderTags = () => {
        ...
        display.push(
            ...cautions.map((caution) => (
                <div key={caution} className={styles.cautionTag}>
                    {caution}
                </div>
            ))
        );
        display.push(
            ...dietLabels.map((diet) => (
                <div key={diet} className={styles.dietTag}>
                    {diet}
                </div>
            ))
        );
        display.push(
            ...healthLabels.map((health) => (
                <div key={health} className={styles.healthTag}>
                    {health}
                </div>
            ))
        );
        ...
        <div className={styles.recipePage}>
            <div className={styles.pageHeader}>
                ...
                <div className={styles.headerInfo}>
                    ...
                    <p>
                        Full recipe available from{" "}
                        <a href={url} target="_blank" rel="noopener noreferrer">
                            {source}
                        </a>
                    </p>
                    <div className={styles.cuisineAndDishTagContainer}>
                        <div>{cuisineTypeDisplay}</div>
                        <div>{dishTypeDisplay}</div>
                    </div>
                    {totalTime > 0 && (
                        <p className={styles.pageCookTime}>
            ...
            <div className={styles.body}>
                <div className={styles.bodyLeftColumn}>
                    <div className={styles.ingredientListContainer}>
                        <h3 className={styles.ingredientListHeading}>
                            Ingredients
                        </h3>
