...
export function subscribeToMealPlans(
    uid: string,
    callback: (data: MealPlans | null) => void
): Unsubscribe {
...
export function subscribeToGroceryList(
    uid: string,
    callback: (data: Ingredient[] | null) => void
): Unsubscribe {
    ...
    return onSnapshot(colRef, (snapshot) => {
        ...
        const data = docList.map((doc) => {
            const food = doc.id;
            const { price, quantity, measure } =
                doc.data() as IngredientDetails;
...
export function subscribeToSavedRecipes(
    uid: string,
    callback: (data: SavedRecipeWithId[] | null) => void
): Unsubscribe {
...
export async function setMeal(
    uid: string,
    date: string,
    data: Partial<Record<"breakfast" | "lunch" | "dinner" | "snack", Meal>>
) {
